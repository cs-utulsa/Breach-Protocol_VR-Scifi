...
public class VRFootIK : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        layer_mask= LayerMask.GetMask("Player","Objective", "Player Hand", "Interactable", "WorldCollision", "Enemy", "Invisible");
    }
    ...
    private void OnAnimatorIK(int layerIndex)
    {
        ...
        bool hasHit = Physics.Raycast(rightFootPos + Vector3.up, Vector3.down, out hit, footDistance, ~layer_mask);
        if(hasHit)
        {
        ...
        hasHit = Physics.Raycast(leftFootPos + Vector3.up, Vector3.down, out hit, footDistance, ~layer_mask);
        if (hasHit)
        {
